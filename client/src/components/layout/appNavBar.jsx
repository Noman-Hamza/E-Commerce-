...
const AppNavBar = () => {
    const navigate = useNavigate();
    const { SearchKeyword, SetSearchKeyword } = useProductStore();
    const { isAuthenticated, checkLoginStatus, UserLogoutRequest } = UserStore();
    const { CartCount, CartListRequest } = CartStore();
                ...
                <div className="container">
                    ...
                    <div className="collapse navbar-collapse" id="navbarNav">
                        <ul className="navbar-nav mx-auto mb-2 mb-lg-0 d-flex gap-2">
                            ...
                            <li className="nav-item position-relative">
                                <Link to="/cart" className="nav-link text-white px-3 rounded bg-success">
                                    <i className="bi bi-bag-fill me-1"></i> Cart
                                    {CartCount > 0 && (
                      ...
                      {CartCount}
                            ...
                            <li className="nav-item position-relative">
                                <Link to="/wish" className="nav-link text-white px-3 rounded bg-warning">
                                    <i className="bi bi-heart-fill me-1"></i> Wish
                                    {WishCount > 0 && (
                                        <span className="position-absolute top-0 start-100 translate-middle badge bg-dark text-white">
                      {WishCount}
                    </span>
                                    )}
                                </Link>
                        ...
                        <div className="d-flex flex-column flex-lg-row align-items-center gap-2 ms-lg-3 mt-3 mt-lg-0">
                            ...
                            {isAuthenticated ? (
                                <>
                                    <UserSubmitButton onClick={onLogout} text="Logout" className="btn btn-outline-light px-4" />
                                    <Link to="/profile" className="btn btn-light text-dark px-4">Profile</Link>
                                </>
                            ) : (
                                <Link to="/login" className="btn btn-light text-dark px-4">Login</Link>
                            )}
                        </div>
                    </div>
                </div>
